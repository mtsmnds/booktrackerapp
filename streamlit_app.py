...
with st.sidebar.form("add_book_form"):
    ...
    if submitted and can_submit:
        ...
        save_data(df)
        st.success(f"Added book: {title} by {author}")
...
# === 4. Charts ===
def create_books_per_year_chart(df):
...
def create_pages_per_week_chart(df):
...
create_books_per_year_chart(df)
create_pages_per_week_chart(df)
